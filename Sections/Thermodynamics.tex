...
\frame{
	...
	\item<1-> Integral Gibbs energy of a multicomponent, multiphase system can be expressed in terms of chemical potentials, $\mu_{i(\lambda)}$, of the species.
		...
		\vspace{-0.15cm}
		\begin{equation*}
        			\frac{G_{sys}}{RT} = \left ( \sum_{\lambda=1}^{\Lambda} n_{\lambda} \sum_{i=1}^{N_{\lambda}}x_{i({\lambda})}\tilde{\mu}_i + \sum_{\omega=1}^{\Omega} n_{\omega} \tilde{\mu}_{\omega} \right )
    		\end{equation*}
		\end{block}
	\item<2-> The Gibbs energy of  the system can also be expressed in terms of the system component potentials, $\Gamma_j$, and the number of moles, $b_j$, of the system components.
		\begin{exampleblock}{}
		\vspace{-0.15cm}
		\begin{equation*}
...
\frame{
	...
	\begin{figure}[htbp]
		\begin{center}
		\includegraphics[height=0.65\paperheight]{Figures/Gibbs_plane}
		\end{center}
	\end{figure}
	\blfootnote{Piro et al., J. Nucl. Mater, 414 (3), 399-407, 2011.}}
}
